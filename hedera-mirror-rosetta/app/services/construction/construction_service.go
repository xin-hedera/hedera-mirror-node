...
import (
	...
	"github.com/hashgraph/hedera-mirror-node/hedera-mirror-rosetta/app/services/handler"
...
func (c *ConstructionAPIService) ConstructionParse(
	...
	operations, accounts, err := c.transactionHandler.ParseConstructedTransaction(transaction, request.Signed)
	if err != nil {
		return nil, err
	}
...
func (c *ConstructionAPIService) ConstructionPayloads(
	...
	return &rTypes.ConstructionPayloadsResponse{
		UnsignedTransaction: hexutils.SafeAddHexPrefix(hex.EncodeToString(bytes)),
...
func addSignature(transaction handler.ITransaction, pubKey hedera.PublicKey, signature []byte) *rTypes.Error {
...
func getFrozenTransactionBodyBytes(transaction handler.ITransaction) ([]byte, *rTypes.Error) {
...
func unmarshallTransactionFromHexString(transactionString string) (handler.ITransaction, *rTypes.Error) {
	...
	iTransaction, ok := transaction.(handler.ITransaction)
	if !ok {
