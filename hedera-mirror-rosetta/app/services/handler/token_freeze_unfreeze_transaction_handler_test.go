...
func TestTokenFreezeUnfreezeTransactionHandlerSuite(t *testing.T) {
	suite.Run(t, new(tokenFreezeUnfreezeTransactionHandlerSuite))
}

type tokenFreezeUnfreezeTransactionHandlerSuite struct {
	suite.Suite
}

func (suite *tokenFreezeUnfreezeTransactionHandlerSuite) TestNewTokenFreezeTransactionHandler() {
	h := newTokenFreezeTransactionHandler(&repository.MockTokenRepository{})
	assert.NotNil(suite.T(), h)
}

func (suite *tokenFreezeUnfreezeTransactionHandlerSuite) TestNewTokenUnfreezeTransactionHandler() {
	h := newTokenUnfreezeTransactionHandler(&repository.MockTokenRepository{})
	assert.NotNil(suite.T(), h)
}

func (suite *tokenFreezeUnfreezeTransactionHandlerSuite) TestGetOperationType() {
	var tests = []struct {
		name       string
		...
		{
			name:       "TokenFreezeTransactionHandler",
			newHandler: newTokenFreezeTransactionHandler,
			expected:   config.OperationTypeTokenFreeze,
		},
		{
			name:       "TokenUnfreezeTransactionHandler",
			newHandler: newTokenUnfreezeTransactionHandler,
...
func (suite *tokenFreezeUnfreezeTransactionHandlerSuite) TestGetSdkTransactionType() {
	var tests = []struct {
		name       string
		newHandler newHandlerFunc
		expected   string
...
func (suite *tokenFreezeUnfreezeTransactionHandlerSuite) TestConstruct() {
	var tests = []struct {
		name             string
	...
	suite.T().Run("TokenFreezeTransactionHandler", func(t *testing.T) {
		runTests(t, config.OperationTypeTokenFreeze, newTokenFreezeTransactionHandler)
	})

	suite.T().Run("TokenUnfreezeTransactionHandler", func(t *testing.T) {
		runTests(t, config.OperationTypeTokenUnfreeze, newTokenUnfreezeTransactionHandler)
	})
}

func (suite *tokenFreezeUnfreezeTransactionHandlerSuite) TestParseConstructedTransaction() {
...
func (suite *tokenFreezeUnfreezeTransactionHandlerSuite) TestParseExecutedTransaction() {
	assert.True(suite.T(), true)
}

func (suite *tokenFreezeUnfreezeTransactionHandlerSuite) TestPreprocess() {
	var tests = []struct {
		name             string
		tokenRepoErr     *rTypes.Error
		updateOperations updateOperationsFunc
