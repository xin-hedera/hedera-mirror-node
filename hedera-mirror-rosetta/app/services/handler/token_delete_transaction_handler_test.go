...
func TestTokenDeleteTransactionHandlerSuite(t *testing.T) {
	suite.Run(t, new(tokenDeleteTransactionHandlerSuite))
}

type tokenDeleteTransactionHandlerSuite struct {
	suite.Suite
}

func (suite *tokenDeleteTransactionHandlerSuite) TestNewTransactionHandler() {
	h := newTokenDeleteTransactionHandler(&repository.MockTokenRepository{})
	assert.NotNil(suite.T(), h)
}

func (suite *tokenDeleteTransactionHandlerSuite) TestGetOperationType() {
	h := newTokenDeleteTransactionHandler(&repository.MockTokenRepository{})
	assert.Equal(suite.T(), config.OperationTypeTokenDelete, h.GetOperationType())
}

func (suite *tokenDeleteTransactionHandlerSuite) TestGetSdkTransactionType() {
	h := newTokenDeleteTransactionHandler(&repository.MockTokenRepository{})
	assert.Equal(suite.T(), "TokenDeleteTransaction", h.GetSdkTransactionType())
}

func (suite *tokenDeleteTransactionHandlerSuite) TestConstruct() {
	var tests = []struct {
		name             string
	...
	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			...
			h := newTokenDeleteTransactionHandler(mockTokenRepo)
...
func (suite *tokenDeleteTransactionHandlerSuite) TestParseConstructedTransaction() {
...
func (suite *tokenDeleteTransactionHandlerSuite) TestParseExecutedTransaction() {
	assert.True(suite.T(), true)
}

func (suite *tokenDeleteTransactionHandlerSuite) TestPreprocess() {
	var tests = []struct {
		name             string
		tokenRepoErr     *rTypes.Error
		updateOperations updateOperationsFunc
