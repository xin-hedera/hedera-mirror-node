...
func TestTokenWipeTransactionHandlerSuite(t *testing.T) {
	suite.Run(t, new(tokenWipeTransactionHandlerSuite))
}

type tokenWipeTransactionHandlerSuite struct {
	suite.Suite
}

func (suite *tokenWipeTransactionHandlerSuite) TestNewTransactionHandler() {
	h := newTokenWipeTransactionHandler(&repository.MockTokenRepository{})
	assert.NotNil(suite.T(), h)
}

func (suite *tokenWipeTransactionHandlerSuite) TestGetOperationType() {
	h := newTokenWipeTransactionHandler(&repository.MockTokenRepository{})
	assert.Equal(suite.T(), config.OperationTypeTokenWipe, h.GetOperationType())
}

func (suite *tokenWipeTransactionHandlerSuite) TestGetSdkTransactionType() {
	h := newTokenWipeTransactionHandler(&repository.MockTokenRepository{})
	assert.Equal(suite.T(), "TokenWipeTransaction", h.GetSdkTransactionType())
}

func (suite *tokenWipeTransactionHandlerSuite) TestConstruct() {
	var tests = []struct {
		name             string
	...
	for _, tt := range tests {
		suite.T().Run(tt.name, func(t *testing.T) {
			...
			h := newTokenWipeTransactionHandler(mockTokenRepo)
...
func (suite *tokenWipeTransactionHandlerSuite) TestParseConstructedTransaction() {
	...
	var tests = []struct {
		...
		},
		{
			...
			getTransaction: func() ITransaction {
				// TODO SDK bug, once SDK is fixed, remove the SetAccountID call
				return hedera.NewTokenWipeTransaction().
...
func (suite *tokenWipeTransactionHandlerSuite) TestParseExecutedTransaction() {
	assert.True(suite.T(), true)
}

func (suite *tokenWipeTransactionHandlerSuite) TestPreprocess() {
	var tests = []struct {
		name             string
		tokenRepoErr     *rTypes.Error
		updateOperations updateOperationsFunc
