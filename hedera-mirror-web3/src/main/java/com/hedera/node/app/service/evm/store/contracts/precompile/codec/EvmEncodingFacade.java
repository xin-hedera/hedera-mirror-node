...
public class EvmEncodingFacade {
    ...
    private static class FunctionResultBuilder {
        ...
        private Bytes build() {
            final var result =
                    switch (functionType) {
                        case ERC_DECIMALS -> Tuple.from(decimals);
                        case ERC_ALLOWANCE -> Tuple.from(BigInteger.valueOf(allowance));
                        case ERC_TOTAL_SUPPLY -> Tuple.from(BigInteger.valueOf(totalSupply));
                        case ERC_BALANCE -> Tuple.from(BigInteger.valueOf(balance));
                        case ERC_IS_APPROVED_FOR_ALL -> Tuple.from(isApprovedForAllStatus);
                        ...
                        case ERC_TOKEN_URI -> Tuple.from(metadata);
                        case ERC_OWNER -> Tuple.from(convertBesuAddressToHeadlongAddress(owner));
                        case ERC_GET_APPROVED -> Tuple.from(convertBesuAddressToHeadlongAddress(approved));
